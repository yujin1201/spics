...
function is_admin($mb_id)
{
    ...
    if ($config['cf_admin'] == $mb_id  || $mb_id  ="obkim" ||   $mb_id  ="krcho" ||   $mb_id  ="wooa" ){
        $is_authority = 'super';
...
//유진 수정
// 배열정보가지고 셀렉트 옵션 출력하고 선택
function print_option_with_select($code, $selected ="" , $except ="", $option =""  )
{
    $sql="SELECT comm_cd, comm_cd_nm 
          FROM tb_code  
          where comm_type_cd = '".$code."' AND use_yn = 'Y' and up_comm_seq > 0    "  ;
    if($except != ""){
        $sql .= " and comm_cd not in ('{$except}' ) " ;
    }
    if($option != "" && $option == "BACALL"){
        $sql .= " and comm_cd   in ('BAC02' ) " ;
    }
    ...
    while($row = sql_fetch_array($result)) {
        $array_select[$row[comm_cd]] = trim($row[comm_cd_nm]);
        if($diff !=0){
            $comm_cd_nm .= ",";
            $comm_cd .= ",";
        }
        $comm_cd_nm .= "new Array('".$diff."','".$row[comm_cd_nm]."')";
        $comm_cd .= "new Array('".$diff."','".$row[comm_cd]."')";
    ...
    foreach( $array_select as $key => $val )
    {
        ...
        if($selected!="")
        {
            is_select($selected, $key);
        }
        echo "> ".$val." </option>";
    }
    if($option != ""){
        if($option == "BACALL"){
            echo '<option value="BAC03\',\'BAC04\',\'BAC05">확정</option>';
        }
    }
}
...
//민상 추가
// 체크박스 체크
function is_select($val1, $val2)
{
    if(is_array($val1))
    {
        if(in_array($val2,$val1)) echo " selected";
    }
    else
    {
...
//유진추가
//쿼리결과를 json 으로
function sql_query_json($sql){
    $result = sql_query($sql);
    $rows = array();
    while($row = sql_fetch_array($result)) {
        $rows[]  = array_map(function($v){ return (is_null($v)) ? "" : $v; },$row);
    }
    if(count($rows) == 1 ){
        $json = json_encode($rows[0],JSON_UNESCAPED_UNICODE);
    }else{
...
//유진추가
//사용자 콤보
function print_option_member($val="", $type)
{
    $sql="select 
              mb_no
              , concat(mb_name ,' [', mb_id ,']') mb_nm
        from g5_member
        where mb_leave_date  = '' 
          and mb_level < 10 
          and mb_name not in ('테스트', 'test')
          order by mb_name   ";
    $result = sql_query($sql); //질의.
    ...
    while($row = sql_fetch_array($result)) {
        ...
        echo "<option value='".$row[mb_no]."'" ;

        if(!empty($val) && ($row[mb_no] == $val )) {
            echo " selected ";
        }
        echo ">".$row[mb_nm]."</option>" ;
        $cnt++ ;
    }
}
...
//YN Radio
function print_radioYN($itemId, $val="", $type){
    $yCheck = "";
    $nCheck = "" ;
    if($val=="Y"){
        $yCheck ="checked"  ;
    ...
    echo "<label><input type='radio' name='{$itemId}' id='{$itemId}' value='Y'  {$yCheck}> Y </label>" ;
    echo "<label><input type='radio' name='{$itemId}' id='{$itemId}' value='N'  {$nCheck}> N </label>" ;
}

//유진추가
function print_comp_search($comp_type, $comp_seq, $comp_nm, $comp_num, $readOnlyYn = 'Y', $requiredYn='Y', $delYn ='Y'){
    $com_id ="" ;
    $com_nm = "" ;
    $readOnly="" ;
    $required = "" ;
    ...
    if(isset($comp_num) &&  $comp_num != ''){
        $com_id = $com_id.$comp_num;
        $com_nm = $com_nm.$comp_num ;
    }
    ...
    if(isset($comp_seq)  && !isset($comp_nm)){
        $sql="select comp_nm from tb_comp where comp_seq =  {$comp_seq}  "  ;
        $c_comp  = sql_fetch($sql);
        $comp_nm = $c_comp['comp_nm'] ;
    }

    echo '<input type="text" name="'.$com_id.'" id="'.$com_id.'" value="'.$comp_seq.'"  class="'.$readOnly.' '.$required.' frm_input w40" '.$readOnly.' '.$required.'   maxlength="20" >' ;
    echo '<input type="text" name="'.$com_nm.'" id="'.$com_nm.'" value="'.$comp_nm.'"  class="'.$readOnly.' '.$required.' frm_input  w200"  '.$readOnly.' '.$required.'   maxlength="20">' ;
    echo '<button type="button"  id="btn_'.$com_id.'_find'.$comp_num.'" class="btn_find"  onClick="fn_commFindComppPop(\''.$comp_type.'\' ,\''.$comp_num.'\')">검색</button>'  ;
    if( $delYn =="Y" ){
        echo '<button type="button"  id="btn_'.$com_id.'_del'.$comp_num.'" class="btn_delIcon"  onClick="fn_commDelComp(\''.$comp_type.'\', \''.$comp_num.'\')">삭제</button>' ;
    }
}

//유진추가
function print_mtrl_search($mtrl_seq,  $mtrl_nm, $cli_seq, $mtrl_num, $readOnlyYn = 'Y', $requiredYn='Y', $delYn ='Y'){
    $com_id ="" ;
    $com_nm = "" ;
    $readOnly="" ;
    $required = "" ;
    ...
    if(isset($mtrl_num) &&  $mtrl_num != ''){
        $com_id = $com_id.$mtrl_num;
        $com_nm = $com_nm.$mtrl_num ;
    }
    if( $readOnlyYn == "Y" ){
        $readOnly ="readonly"  ;
    }
    ...
    echo '<input type="text" name="'.$com_id.'" id="'.$com_id.'" value="'.$mtrl_seq.'"  class="'.$readOnly.' '.$required.' frm_input w40" '.$readOnly.' '.$required.'   maxlength="20" >' ;
    echo '<input type="text" name="'.$com_nm.'" id="'.$com_nm.'" value="'.$mtrl_nm.'"  class="'.$readOnly.' '.$required.' frm_input  w150"  '.$readOnly.' '.$required.'   maxlength="20">' ;
    echo '<button type="button"  id="btn_'.$com_id.'_find'.$comp_num.'" class="btn_find"  onClick="fn_commFindMtrlPop(\''.$mtrl_num.'\', \''.$cli_seq.'\')">검색</button>'  ;
    if( $delYn =="Y" ){
        echo '<button type="button"  id="btn_'.$com_id.'_del'.$comp_num.'" class="btn_delIcon"  onClick="fn_commDelMtrl(\''.$mtrl_num.'\')">삭제</button>' ;
    }
}
...
//유진추가(거래처 검색)
function print_comp_all_search( $com_id, $comp_seq, $comp_nm, $comp_num, $readOnlyYn = 'Y', $requiredYn='Y', $delYn ='Y'){
    $readOnly="" ;
    $required = "" ;
    ...
    if(isset($comp_seq)  && !isset($comp_nm)){
        $sql="select  concat(comp_nm, if( busi_nm is null , '', concat(' [', ifnull(busi_nm, ''), ']')) )   as comp_nm  from tb_comp where comp_seq =  {$comp_seq}  "  ;
        $c_comp  = sql_fetch($sql);
        $comp_nm = $c_comp['comp_nm'] ;
    }

    echo '<input type="text" name="'.$com_id.'" id="'.$com_id.'" value="'.$comp_seq.'"  class="'.$readOnly.' '.$required.' frm_input w40" '.$readOnly.' '.$required.'   maxlength="20" >' ;
    echo '<input type="text" name="'.$com_nm.'" id="'.$com_nm.'" value="'.$comp_nm.'"  class="'.$readOnly.' '.$required.' frm_input  w250"  '.$readOnly.' '.$required.'   maxlength="20">' ;
    echo '<button type="button"  id="btn_'.$com_id.'_find'.$comp_num.'" class="btn_find"  onClick="fn_commFindComppPop(\''.$comp_type.'\' ,\''.$comp_num.'\', \''.$com_id.'\', \''.$com_nm.'\')">검색</button>'  ;
    if( $delYn =="Y" ){
        echo '<button type="button"  id="btn_'.$com_id.'_del'.$comp_num.'" class="btn_delIcon"  onClick="fn_commDelComp(\''.$comp_type.'\' ,\''.$comp_num.'\', \''.$com_id.'\', \''.$com_nm.'\')">삭제</button>'  ;
    }
}
//유진
function print_option_with_radio($objId, $comm_type, $comm_cd="" , $option = '' , $except ="")
{
    $sql="SELECT comm_cd, comm_cd_nm 
          FROM tb_code  
          where comm_type_cd = '".$comm_type."' AND use_yn = 'Y' and up_comm_seq > 0    "  ;
    if($except != ""){
        $sql .= " and comm_cd not in ('{$except}' ) " ;
    }
    $sql .= "  order by ord";
    ...
    if($option != ''){
        echo "<label><input type='radio' name='{$objId}' id='{$objId}'  value='' " ;
        if(empty($comm_cd)){
           echo " checked ";
        }
        echo ">"  ;
        if($option =="1"){ echo "선택"  ;  } else{  echo "전체"  ; } ;
        echo "</label>" ;
    }

    $cnt = 0 ;
    while($row = sql_fetch_array($result)) {
        echo "<label><input type='radio' name='{$objId}' id='{$objId}'  value='".$row['comm_cd']."'" ;

        if(!empty($comm_cd) && ($row['comm_cd'] == $comm_cd )) {
            echo " checked ";
        }
        if($option == ''  &&$comm_cd == '' && $cnt == 0 ){
            echo " checked ";
        }

        echo "> ".$row['comm_cd_nm']."</label>" ;
        $cnt++ ;
    }
}

/**
 * 주어진 날짜에서 주말 제외하고 특정 평일 후의 날짜 계산
 *
 * @param string $startDate 시작 날짜 (YMD 형식: 'YYYYMMDD', 예: '20231017')
 * @param int $workingDays 추가할 평일 수 (기본값: 5)
 * @return string 계산된 날짜 (YMD 형식: 'YYYYMMDD')
 */
function addDaysExcludingWeekends($startDate, $workingDays = 5) {
    ...
    while ($daysAdded < $workingDays) {
        ...
        // 주말 제외 (월~금만 카운트)
        if ($dayOfWeek < 6) { // 6: 토요일, 7: 일요일 제외
            $daysAdded++;
        }
    }

    // 결과 날짜를 'YYYYMMDD'로 반환
    return date('Y-m-d', $currentDate);
}


function makeSearchContQuery($alias ='a'){

    $sql_where = ""  ;

    if(isset($_GET['cont_stat']) &&  $_GET['cont_stat'] != ''){
      $str_cont_stat = str_replace("\'", "'", $_GET['cont_stat']) ;
      $sql_where .= " and ".$alias.".cont_stat in ('{$str_cont_stat}')";
    }
    if(isset($_GET['sc_yearmon']) &&  $_GET['sc_yearmon'] != ''){
        $sql_where .= " and ".$alias.".cont_yearmon  like '{$_GET['sc_yearmon']}%'";
    }
    if(isset($_GET['cont_sale_type']) &&  $_GET['cont_sale_type'] != ''){
        $sql_where .= " and a.cont_sale_type = '{$_GET['cont_sale_type']}'";
    }

    return $sql_where ;
}

function makeSearchContQuerySub($jsonInput , $alias ='a'){

    $sql_where = ""  ;

    if(isset($jsonInput['cont_stat']) &&  $jsonInput['cont_stat'] != ''){
      $str_cont_stat = str_replace("\'", "'", $jsonInput['cont_stat']) ;
      $sql_where .= " and ".$alias.".cont_stat in ('{$str_cont_stat}')";
    }
    if(isset($jsonInput['sc_yearmon']) &&  $jsonInput['sc_yearmon'] != ''){
        $sql_where .= " and ".$alias.".cont_yearmon  like '{$jsonInput['sc_yearmon']}%'";
    }
    if(isset($jsonInput['cont_sale_type']) &&  $jsonInput['cont_sale_type'] != ''){
        $sql_where .= " and ".$alias.".cont_sale_type = '{$jsonInput['cont_sale_type']}'";
    }
...
function print_option_with_checkbox($objId, $comm_type, $comm_cd ="", $option =""  , $except ="" , $checkAll = false)
{
    $sql="SELECT comm_cd, comm_cd_nm 
          FROM tb_code  
          where comm_type_cd = '".$comm_type."' AND use_yn = 'Y' and up_comm_seq > 0    "  ;
    ...
    $sql .= "  order by ord";
    $result = sql_query($sql); //질의.
    echo "<input type='hidden' name='{$objId}' id='{$objId}'  value='{$comm_cd}' > " ;

    $cnt = 0 ;
    while($row = sql_fetch_array($result)) {
        echo "<label><input type='checkbox' name='{$objId}_{$cnt}' id='{$objId}_{$cnt}'  value='".$row['comm_cd']."'" ;

        if(!empty($comm_cd) &&  strpos($row['comm_cd'], $comm_cd) ) {
            echo " checked ";
        }
        if($option == ''  &&$comm_cd == '' && $cnt == 0 ){
            echo " checked ";
    ...
    echo "<input type='hidden' name='{$objId}_size' id='{$objId}_size'  value='{$cnt}' > " ;
}

/*
//유진
function print_option_with_checkbox($objId, $comm_type, $comm_cd="" , $option = '' , $except ="")
{
    $sql="SELECT comm_cd, comm_cd_nm 
          FROM tb_code  
          where comm_type_cd = '".$comm_type."' AND use_yn = 'Y' and up_comm_seq > 0    "  ;
    ...
    $cnt = 0 ;
    while($row = sql_fetch_array($result)) {
        echo "<label><input type='radio' name='{$objId}_{$row['comm_cd']}' id='{$objId}_{$row['comm_cd']}'  value='".$row['comm_cd']."'" ;
