...
if ($w == 'I'){
    ...
    //최초 입력시 사업자 번호 중복 체크
    $sql = "select  count(*) as cnt   from tb_comp where del_yn='N' and comp_type = 'AAC04'
    ...
    $result = sql_query(" insert into tb_comp set entr_dt=now(),entr_prsn ='{$member['mb_no']}',comp_type = 'AAC04', {$sql_common} ");
    if($result)  $last_seq_no = sql_insert_id();

    //alert("등록  했습니다.", './board_copy.php?bo_table='.$bo_table.'&amp;'.$qstr);
    goto_url("./rep_form.php?w=u&comp_seq={$last_seq_no}");
}else if($w == 'U'){

    $result = sql_query(" update tb_comp set updt_dt=now(),updt_prsn ='{$member['mb_no']}',comp_type = 'AAC04', {$sql_common} where comp_seq = '{$_POST['comp_seq']}' ");

    goto_url("./rep_form.php?w=u&comp_seq={$_POST['comp_seq']}");
    ...
    //계약에 정보가 존재하면 삭제 하면 안됨

    $sql = "SELECT count(*) as cnt     
       FROM tb_cont a  where a.rep_seq='{$comp_seq}'";

    $cnt = sql_fetch($sql);

    if($cnt['cnt'] > 0){
        alert("계약 정보가 존재 합니다. 삭제 할 수 없습니다.", './rep_form.php?w=u&comp_seq='.$comp_seq);
    }else{
        $result = sql_query(" update tb_comp set updt_dt=now(),updt_prsn ='{$member['mb_no']}',del_yn = 'Y' where comp_seq = '{$comp_seq}' ");
        alert("삭제 완료", './rep_list.php');
    }
}
