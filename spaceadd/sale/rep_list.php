...
$sql = " select sum(case when deal_sts_code='BAA01' then 1 else 0 end) as sts_ok,
            sum(case when deal_sts_code !='BAA01' then 1 else 0 end) as sts_stop
             from tb_comp where comp_type='AAC04' ";
$row = sql_fetch($sql);
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#grid").jqxGrid(
            {
                ...
                columns: [
                    ...
                    { text: '미디어랩코드', datafield: 'comp_seq', filtertype: 'checkedlist', cellsalign: 'right', align: 'center'  ,width:70},
                    { text: '미디어랩명', datafield: 'comp_nm', filtertype: 'checkedlist', cellsalign: 'center', align: 'center'  ,width:170},
                    ...
                    { text: '결재조건', datafield: 'bill_type_nm', filtertype: 'checkedlist' , cellsalign: 'center', align: 'center'  ,width:120},
                    { text: '거래상태', datafield: 'deal_sts_nm', filtertype: 'checkedlist' , cellsalign: 'center', align: 'center'  ,width:120},
                    { text: '거래발생일자', datafield: 'deal_ocur_dt', filtertype: 'checkedlist' , cellsalign: 'center', align: 'center'  ,width:100},
        ...
        $('#grid').on('rowdoubleclick', function (event) {
            ...
            location.href = g_sale_url+"/rep_form.php?w=u&comp_seq="+getRowData['comp_seq'];
        });

    });

    function fn_add_comp(){
        location.href = g_sale_url+"/rep_form.php";
    }

</script>
<div class="local_ov01 local_ov">
    <a href="?sfl=<?php echo $sfl ?>&amp;stx=<?php echo $stx ?>&amp;deal_sts_code=" class="btn_ov01" data-tooltip-text="모든 미디어랩사를 조회합니다."><span class="btn_ov01"><span class="ov_txt">총 미디어랩 수 </span><span class="ov_num"> <?php echo number_format($row[sts_ok]+$row[sts_stop]) ?> </span></span></a>
    <a href="?sfl=<?php echo $sfl ?>&amp;stx=<?php echo $stx ?>&amp;deal_sts_code=BAA01" class="btn_ov01" data-tooltip-text="거래상태 정상 미디어랩사를 조회합니다."> <span class="ov_txt">정상 </span><span class="ov_num"><?php echo number_format($row[sts_ok]) ?>건</span></a>
    <a href="?sfl=<?php echo $sfl ?>&amp;stx=<?php echo $stx ?>&amp;deal_sts_code=BAANOT" class="btn_ov01" data-tooltip-text="거래상태 정상이 아닌 미디어랩사를 조회합니다."> <span class="ov_txt">거래불가  </span><span class="ov_num"><?php echo number_format($row[sts_stop]) ?>건</span></a>
</div>

<form id="fsearch" name="fsearch" class="local_sch01 local_sch" method="get">
    <label for="sfl" class="sound_only">검색대상</label>
    <select name="sfl" id="sfl">
        <option value="comp_nm"<?php echo get_selected($sfl, "comp_nm"); ?>>미디어랩명</option>
...
<div class="btn_fixed_top">
    <button type="button" onclick="fn_add_comp();" class="btn btn_02">미디어랩 등록 <span class="sound_only"> 새창</span></button>
