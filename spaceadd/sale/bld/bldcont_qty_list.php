    ...
    <script type="text/javascript">
        ...
        function generateData() {
            var str= './bld/bldcont_qty_list_result.php'
        ...
        function markGrid(data){
            ...
            var coloumns =[
                ...
                { text: '매체타입', datafield: 'bld_mda_type_nm', filtertype: 'checkedlist', cellsalign: 'center', align: 'center'  ,width:100 , pinned: true},
            ...
            const contSaleType = document.getElementById("cont_sale_type").selectedIndex  ;

            let currentDate = startDate.clone(); // 시작 날짜를 복사
            let preDate = ""; // 시작 날짜를 복사

            const cellclass = function (index, datafield, value, defaultvalue, column, rowdata) {
                /*console.log (index, datafield, value, defaultvalue, column, rowdata)*/
                var _ableCnt = 999   ;
                if (sec_div == "able_sec" ) {
            ...
            while (currentDate.compareTo(endDate) <= 0) { // 종료날짜까지 반복
                ...
                coloumns.push(col)   ;
                datafields.push({name :"c"+currentDate.toString("yyyyMMdd"),  type :  "string"  }) ;
                datafields.push({name :"a"+currentDate.toString("yyyyMMdd"),  type :  "number"  }) ;
                datafields.push({name :"l"+currentDate.toString("yyyyMMdd"),  type :  "number"  }) ;
                datafields.push({name :"m"+currentDate.toString("yyyyMMdd"),  type :  "number"  }) ;
            ...
            data.forEach((el)=>{
                if(!dataSource.find((it)=>el.bld_seq === it.bld_seq)){
                    var item = { "bld_seq": el.bld_seq , "bld_nm": el.bld_nm , "bld_num": el.bld_num, "ins_sec": el.ins_sec, "ins_cnt": el.ins_cnt , "bld_mda_type_nm" :el.bld_mda_type_nm  } ;
                    data.filter(tt=> tt.bld_seq === el.bld_seq ).forEach((bld)=>{
                       ...
                       if(sec_div == "able_sec") val =  bld.able_sec  ;
                        ...
                        item["c"+bld.dt] = val +""+( (Number(bld.local_sec) > 0  && contSaleType === 0 ) ? "("+bld.local_sec +")" :"" ) ;
                        item["a"+bld.dt] = bld.able_sec  ;
                        item["l"+bld.dt] = bld.local_sec   ;
                        item["m"+bld.dt] = bld.mtrl_sec   ;
            ...
            {
                var args = event.args;
                var _date =  (args.datafield.indexOf("c") === 0 ) ? args.datafield.replaceAll("c" , "") : "" ;
                url = "../bld_cont_qty_pop.php?bld_seq="+args.row.bounddata['bld_seq']
                    +"&st_dt="+((_date === "" ) ? document.getElementById("st_dt").value.replaceAll("-" , "") :  _date)
                    +"&ed_dt="+((_date === "" ) ? document.getElementById("ed_dt").value.replaceAll("-" , "") :  _date)
...
?>
    ...
    <form id="fsearch" name="fsearch" class="local_sch01 local_sch" method="get" action="bldcont_qty_list.php">
