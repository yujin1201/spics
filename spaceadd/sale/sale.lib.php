...
// 회원권한을 SELECT 형식으로 얻음
function get_spin_select($name, $start_id=0, $end_id=10, $selected="", $add_value="")
{
    global $g5;

    $str = "\n<select id=\"{$name}\" name=\"{$name}\"";
    if (!$add_value) $add_value = 1;
    $str .= ">\n";
    for ($i=$start_id; $i<=$end_id; $i = $i+$add_value) {
        $str .= '<option value="'.$i.'"';
        if ($i == $selected)
            $str .= ' selected="selected"';
        $str .= ">{$i}</option>\n";
...
if (!$member['mb_id'])
{
    ...
    alert('로그인 하십시오.', G5_BBS_URL.'/login.php?url=' . urlencode(correct_goto_url($url)));
}
...
unset($menu);
unset($amenu);
$tmp = dir(G5_SALE_PATH);
$menu_files = array();
while ($entry = $tmp->read()) {
    if (!preg_match('/^sale.menu([0-9]{3}).*\.php$/', $entry, $m))
        continue;  // 파일명이 menu 으로 시작하지 않으면 무시한다.

    $amenu[$m[1]] = $entry;
    $menu_files[] = G5_SALE_PATH.'/'.$entry;
}
@asort($menu_files);
