...
$sql = " select sum(case when deal_sts_code='BAA01' then 1 else 0 end) as sts_ok,
            sum(case when deal_sts_code !='BAA01' then 1 else 0 end) as sts_stop
             from tb_comp where comp_type='AAC03' and del_yn='N' ";
$row = sql_fetch($sql);
...
<script type="text/javascript">
        ...
        $("#grid").jqxGrid(
            {
                ...
                columns: [
                    ...
                    { text: '광고회사코드', datafield: 'comp_seq', filtertype: 'checkedlist', cellsalign: 'right', align: 'center'  ,width:70},

                    { text: '광고회사명', datafield: 'comp_nm', filtertype: 'checkedlist', cellsalign: 'center', align: 'center'  ,width:170},
    ...
    function fn_add_comp(){
        location.href = g_sale_url+"/agncy_form.php";
    }

</script>
<div class="local_ov01 local_ov">
    <a href="?sfl=<?php echo $sfl ?>&amp;stx=<?php echo $stx ?>&amp;deal_sts_code=" class="btn_ov01" data-tooltip-text="모든 광고회사를 조회합니다."><span class="btn_ov01"><span class="ov_txt">총 광고회사 수 </span><span class="ov_num"> <?php echo number_format($row[sts_ok]+$row[sts_stop]) ?> </span></span></a>
    <a href="?sfl=<?php echo $sfl ?>&amp;stx=<?php echo $stx ?>&amp;deal_sts_code=BAA01" class="btn_ov01" data-tooltip-text="거래상태 정상 광고회사를 조회합니다."> <span class="ov_txt">정상 </span><span class="ov_num"><?php echo number_format($row[sts_ok]) ?>건</span></a>
    <a href="?sfl=<?php echo $sfl ?>&amp;stx=<?php echo $stx ?>&amp;deal_sts_code=BAANOT" class="btn_ov01" data-tooltip-text="거래상태 정상이 아닌 광고회사를 조회합니다."> <span class="ov_txt">거래불가  </span><span class="ov_num"><?php echo number_format($row[sts_stop]) ?>건</span></a>
</div>
...
<form id="fsearch" name="fsearch" class="local_sch01 local_sch" method="get">
    ...
    <select name="sfl" id="sfl">
        ...
        <option value="comp_nm"<?php echo get_selected($sfl, "comp_nm"); ?>>광고회사명</option>
...
<div class="btn_fixed_top">
    ...
    <button type="button" onclick="fn_add_comp();" class="btn btn_02">광고회사 등록 <span class="sound_only"> 새창</span></button>
