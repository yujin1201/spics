...
$sql = "SELECT comp_seq, comp_nm,(select comm_cd_nm from tb_code where comm_cd = comp_type) as comp_type_nm,
 comp_type, busi_no, corp_no, rep_nm1, rep_nm2, tel_no, fax_no, zipcode, addr1, addr2, busi_sts, item, chrg_nm, chrg_no, chrg_email, psrn_nm, psrn_no, psrn_email, fin_nm, fin_no, fin_email
, deal_sts_code, (select comm_cd_nm from tb_code where comm_cd = deal_sts_code) as deal_sts_nm ,  deal_ocur_dt, rep_indst_div
, bill_type, (select comm_cd_nm from tb_code where comm_cd = bill_type) as bill_type_nm,  bigo, entr_prsn, entr_dt, updt_prsn, updt_dt, mda_type FROM tb_comp where 1=1 ";
